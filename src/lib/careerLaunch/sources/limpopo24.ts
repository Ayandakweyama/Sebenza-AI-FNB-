...
function normalizeUrl(url: string) {
  ...
  if (u.startsWith('/')) return `https://limpopo24.co.za${u}`;
  return `https://limpopo24.co.za/${u.replace(/^\.?\//, '')}`;
}

function pageForType(type: OpportunityType) {
  if (type === 'Bursary') return 'https://limpopo24.co.za/category/bursaries-in-south-africa-for-2026/';
  if (type === 'Entry-Level Job') return 'https://limpopo24.co.za/category/jobs-and-careers-in-south-africa/';
  return 'https://limpopo24.co.za/category/jobs-and-careers-in-south-africa/internships-and-graduate-programmes/';
}
...
function parsePosts(html: string, limit: number) {
  ...
  while ((m = re.exec(html)) && out.length < limit) {
    ...
    const block = html.slice(Math.max(0, m.index), Math.min(html.length, re.lastIndex + 2200));
    ...
    const location =
      ...
      blockText.match(/\bJob Location:\s*([^.\n]+)\b/i)?.[1] ||
      null;
...
export async function scrapeLimpopo24(type: OpportunityType, limit: number) {
