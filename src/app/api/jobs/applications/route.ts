...
async function getUserFromRequest(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log('⚠️ Session auth failed, trying token auth:', sessionError);
    }
...
export async function GET(request: NextRequest) {
  try {
    console.log('🚀 GET /api/jobs/applications - Request received');
    console.log('🔐 Request headers:', Object.fromEntries(request.headers.entries()));
    
    const clerkId = await getUserFromRequest(request);
    
    if (!clerkId) {
      console.log('❌ GET /api/jobs/applications - No user ID found');
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }

    console.log('✅ GET /api/jobs/applications - User authenticated:', clerkId);
